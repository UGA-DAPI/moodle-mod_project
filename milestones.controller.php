...
if ($work == 'dodelete' OR $work == 'delete') {
    ...
    project_tree_delete($milestoneid, 'project_milestone', 0); 
        // uses list option switch
	...
	*/
	if(has_capability('mod/project:validatemilestone', $context)){
		...
		";
		$DB->execute($query);
        project_notify_milestone_change($project, $milestoneid, 3 , $cm->id, $currentGroupId);
            //Alerte email aux étudiants que l'étape a été validé
        $url = $CFG->wwwroot.'/mod/project/view.php?view=milestones&id='.$cm->id;
    }
    redirect($url);

} elseif ($work == 'refuser') {
        //demande de corrections pour une étape
    if(has_capability('mod/project:validatemilestone', $context)){
        $milestoneid = required_param('milestoneid', PARAM_INT);
        $query = "
        UPDATE
    	...
    	//Alerte email aux étudiants que l'étape a été mise en révision
        project_notify_milestone_change($project, $milestoneid, 2 , $cm->id, $currentGroupId);
    ...
    }elseif($work == 'askvalider'){
    //Demande de validation d'une étape
        if(has_capability('mod/project:askvalidatemilestone', $context)){
            ...
            $newMessage->ordering = project_tree_get_max_ordering_message($project->id, $currentGroupId, 'project_messages', true, 0) + 1;
            $returnid = $DB->insert_record('project_messages', $newMessage);
            //Alerte email aux enseignants que une demande de validation est faite pour l'étape      
			project_notify_milestone_change($project, $milestoneid, 1 , $cm->id, $currentGroupId);
			// zip generation
			$zip = new ZipArchive();
			$numDigit = str_pad($numVersion,3, "0",STR_PAD_LEFT);
            //passage du numéro de version sur 3 digit
