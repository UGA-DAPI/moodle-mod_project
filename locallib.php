...
if(count($ressources)>0){
    foreach($ressources as $ressource){
		...
		*/
        $abstract= "<a href=\"view.php?id={$cmid}&amp;work=update&amp;delivid={$ressource->id}&amp;view=deliverables&amp;typeelm=0\">".$ressource->abstract."</a>";
...
if(count($deliverables)>0){
    foreach($deliverables as $deliverable){
        ...
        $abstract= "<a href=\"view.php?id={$cmid}&amp;work=update&amp;delivid={$deliverable->id}&amp;view=deliverables&amp;typeelm=1\">".$deliverable->abstract."</a>";
        echo "<td>".$abstract."</td></tr>";
    }
...
if ($canedit) {
    ...
    <img src=\"".$OUTPUT->pix_url('/p/newnode', 'project')."\" alt=\"".get_string('addsubdeliv', 'project')."\" /></a>";
    $actionsDiscu .= "<a href=\"view.php?id={$cmid}&amp;work=update&amp;messageid={$discussion->id}&amp;view=messages\"><img src=\"".$OUTPUT->pix_url('/t/edit')."\" alt=\"".get_string('update')."\" /></a>";
    /*$actionsDiscu .= "<a href=\"view.php?id={$cmid}&amp;work=delete&amp;messageid={$discussion->id}&amp;view=messages\"><img src=\"".$OUTPUT->pix_url('/t/delete')."\" alt=\"".get_string('delete')."\" /></a>";*/
    ...
    <img src=\"".$OUTPUT->pix_url('/t/down')."\" alt=\"".get_string('down', 'project')."\" /></a>";
    /*if ($discussion->parent != 0) $actionsDiscu .= "<a href=\"view.php?id={$cmid}&amp;work=left&amp;messageid={$discussion->id}&amp;view=messages#node{$discussion->id}\"> <img src=\"".$OUTPUT->pix_url('/t/left')."\" alt=\"".get_string('left', 'project')."\" /></a>";*/
    /*if ($discussion->ordering > 1) $actionsDiscu .= "<a href=\"view.php?id={$cmid}&amp;work=right&amp;messageid={$discussion->id}&amp;view=messages#node{$discussion->id}\"> <img src=\"".$OUTPUT->pix_url('/t/right')."\" alt=\"".get_string('right', 'project')."\" /></a>";*/
    echo "<tr><td colspan='5' align='left'>".$actionsDiscu."</td></tr>";

}
}
function project_print_projects($project, $course){
    global $CFG, $USER, $DB, $OUTPUT;
    $query = "
    SELECT 
    d.*
    ...
    d.timecreated
    ";
    echo "<h3 id='vueprojetstitle'><img src='".$OUTPUT->pix_url('vue-projets', 'project')."' alt='' />".get_string('vueprojetstitle', 'project')."</h3>";
    if ($projects = $DB->get_records_sql($query)) {
        if(count($projects>0)){
            echo "<div id='projectsheader' class='projectitem'><p>Nom du projet</p><p>Etape</p><p>Etape en cours et statut</p><p>Dernier livrable</p><p>&nbsp;</p><div class='sepbloc'></div></div>";
            foreach($projects as $projectitem){
                ...
                $projectName = $projectitem->name;
                    ...
                    if($projectitem->etat==0 && $projetfini){
                        echo "<p><span style=\"cursor:pointer;display:inline;\" onclick=\"javascript:var r=confirm('Etes vous sure de vouloir clore le projet ".addslashes($projectitem->name)." ?');if (r==true){window.location.href='{$CFG->wwwroot}/mod/project/view.php?id={$course}&amp;view=projects&amp;work=close&amp;projectid=".$projectitem->id."';}\">Clore le projet</span></p>";
                    }elseif($projectitem->etat==0 && isset($cmtmp)){
                        $context = context_module::instance($cmtmp->id);
                        $fs = get_file_storage();
                        $files = $fs->get_area_files($context->id, 'mod_project', 'deliverablearchive', false, 'sortorder DESC, timemodified ASC', false);
                        if (!empty($files)) {
                            echo "<a href=\"".project_export_last_deliverable($projectitem,$cmtmp)."\">".get_string('exportdeliv','project')."</a>";
                        }
                        else{
                            echo "<p>&nbsp;</p>";
                        }
...
*/
function project_print_projects_xml($cmid){
    global $CFG, $DB;

    $xml = new DOMDocument('1.0', 'utf-8');

    $recs = $DB->get_records('project',array('course' => $cmid));
    //print_r($recs);
    ...
    foreach ($recs as $rec) {
        ...
        $contexttmp;
        if($cmtmp = get_coursemodule_from_instance('project', $rec->id)){ 
            //print_r($cmtmp);
        //parfois impossible de récuperer le cm associé au projet... et donc pas de lien possible
            $contexttmp = context_module::instance($cmtmp->id);
			//$item->link = $CFG->wwwroot."/mod/project/view.php?id=".$contexttmp->id."&view=description";
            $lien = $CFG->wwwroot."/mod/project/view.php?view=description&amp;id=".$cmtmp->id;
...
function get_users_by_role($cm, $role, $group = '', $parent = false, $all=NULL) {
    ...
    $fields = 'u.id, u.confirmed, u.username, u.email, '. $allnames ;
    $context = context_module::instance($cm->id);
    //allow you to hardcode the role id, but it's dirty coding
    if (gettype($role)=='integer') {
        $roleid = array($role);
    }
    else{
        $roleid = array_keys( get_archetype_roles($role));
    }
    $parentcontexts = '';
    if ($parent) {
    ...
    $sql = "SELECT DISTINCT $fields, ra.roleid FROM {role_assignments} ra
    JOIN {user} u ON u.id = ra.userid JOIN {role} r ON ra.roleid = r.id $ejoin
...
function project_export_last_deliverable($project,$cmid=null){
    ...
    if (!isset($cmid)) {
        //for creating a link in description.php. project contain the cmid so it's fine
        $context = context_module::instance($project->cmid);
    }
    else{
        //for creating the link in project_print_projects. i have to manually pass the cmid.
        $context = context_module::instance($cmid->id);
    }
    $fs = get_file_storage();
    $files = $fs->get_area_files($context->id, 'mod_project', 'deliverablearchive', false, 'sortorder DESC, timemodified ASC', false);
    $file = array_pop($files);
    //on prend le dernier fichier archive
    $path = '/'.$context->id.'/mod_project/deliverablearchive/'.$file->get_itemid().$file->get_filepath().$file->get_filename();
    $url = moodle_url::make_file_url('/pluginfile.php', $path, '');
    return $url;
}
